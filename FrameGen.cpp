...
const bool FrameGen::check(const std::string& framename) {
    // Open frame.
    std::ifstream frame(framename, std::ios::binary);
    if(!frame) {
        std::cout << "Error (check()): could not open frame " << framename << "." << std::endl;
        return false;
    }
    ...
    for(int i=0; i<4; i++) {
        ...
        std::cout << "Frame " << framename << ", COLDATA block " << i+1 << "/4 contains an error in checksum A." << std::endl;
        if(checksum_B(i, _binaryData[3+i*28]>>8))
        std::cout << "Frame " << framename << ", COLDATA block " << i+1 << "/4 contains an error in checksum B." << std::endl;
    }
    if(CRC32(_binaryData[115])) {
        std::cout << "Frame " << framename << " failed its cyclic redundancy check." << std::endl;
        frame.close();
        return false;
    }
    ...
    std::cout << "Warning: Capture error bit set in frame " << framename << "." << std::endl;
    if(_binaryData[2]>>20 & 1) // ASIC
    std::cout << "Warning: ASIC error bit set in frame " << framename << "." << std::endl;
    if(_binaryData[2]>>24 & 1) // WIB_Errors
    std::cout << "Warning: WIB error bit set in frame " << framename << "." << std::endl;
    for(int i=0; i<4; i++) {
        if(_binaryData[3+i*28]>>16 & 1) // S1
        std::cout << "Warning: S1 error bit set in frame " << framename << ", block " << i+1 << "/4." << std::endl;
        if(_binaryData[3+i*28]>>20 & 1) // S2
        std::cout << "Warning: S2 error bit set in frame " << framename << ", block " << i+1 << "/4." << std::endl;
    }
